...
run_kmeans_clustering <- function(subset.dat, npc = 10, km_nstart = 20, km_iter.max = 2500){
  ...
  km_cluster <- kmeans(pcs, 2, nstart = km_nstart, iter.max = km_iter.max ,algorithm  = "Lloyd")$cluster
  return(km_cluster)
...
lightHIPPO <- function(dat, K.round = 10, initial.labels = NULL, initial.round = 0, stop_at = 500, correctByK = FALSE, override.Zscore.cutoff = NULL, smallest.cluster.num = 200, random.num = 2500, move.by.inflation = TRUE){
    ...
    if(is.null(initial.labels) & initial.round > 0) {
      ...
      for(i.round in (initial.round + 1):K.round){
        ...
        if(i.round == 1){
          ...
          if (flag[1]|flag[2]){
            #add blacklist Indicates that the cluster does not need to be divided
  ...
  print(table(res$next_round_IDs))
  return(res)
